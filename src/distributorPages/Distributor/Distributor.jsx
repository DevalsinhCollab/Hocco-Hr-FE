...
import { downloadDisExcel, downloadEmployeeExcel } from "../../utils/utils";
...
import {
  ...
  getAllDistributors,
} from "../../features/DistributorSlice";
import DistributorEditModal from "../../components/Modal/DistributorEditModal";
import ViewDistributorDoc from "../../components/Modal/ViewDistributorDoc";
import { sendAgreementToDistributor } from "../../features/DistributorDocumentSlice";

export default function Distributor() {
  const dispatch = useDispatch();

  const { error } = useSelector((state) => state.userData);
  const { distributors, loading, totalCount } = useSelector(
    (state) => state.distributorData
  );
  ...
  const handleAgreementSend = async (data) => {
    const response = await dispatch(
      sendAgreementToDistributor({ custCode: data.custCode })
    );

    if (response && response.type === "sendAgreementToDistributor/fulfilled") {
      toast.success(response.payload.message);
  ...
  const handleDelete = async (id) => {
    const response = await dispatch(deleteDistributor(id));

    if (response && response.type === "deleteDistributor/fulfilled") {
      toast.success(response.payload.message);
      dispatch(
        getAllDistributors({
          page,
  ...
  const downloadFile = () => {
    const fileName = "Distributor.csv";
  ...
  const _handleCSVToUser = async () => {
    if (jsonData.length > 0) {
      let data = await dispatch(addMultiDistributor(jsonData));

      if (data && data.type == "addMultiDistributor/rejected") {
  ...
  const handleDownload = async () => {
    ...
    const response = await dispatch(DistributorExcelDownload(finalObject));

    let disData = response && response.payload && response.payload.data;
    ...
    const ws = XLSX.utils.json_to_sheet(downloadDisExcel(disData));
    ...
    link.download = "Distributor Data.xlsx";
    link.click();
        ...
        <div className="teamMainBox">
          ...
          <div className="card m-3 p-3">
            <div className="mb-3 d-flex align-items-center justify-content-between">
              <h4 className="m-0">{t("Distributor")}</h4>
              ...
              <DataGrid
                ...
                initialState={{
                  ...(distributors &&
                    ...
                    distributors.initialState),
                  pagination: {
                    ...(distributors &&
                      distributors.length > 0 &&
                      distributors.initialState &&
                      distributors.initialState?.pagination),
                    paginationModel: {
                      pageSize: pageSize,
                    },
                  },
