...
import { updateCfa } from "../../features/CfaSlice";

const CfaEditModal = ({ show, setShow, callApi }) => {
  ...
  const handleSubmit = async () => {
    ...
    const response = await dispatch(updateCfa(finalData));

    if (response && response.payload && response.payload.success) {
      toast("Cfa successfully updated");
    }
    ...
    <>
      <Modal show={show[0]} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Edit Cfa</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group>
